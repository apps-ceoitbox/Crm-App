...
const markdownStyles = {
    ...
    heading1: { fontSize: ms(18), fontWeight: '700', color: Colors.textPrimary, marginVertical: 8 },
    heading2: { fontSize: ms(16), fontWeight: '700', color: Colors.textPrimary, marginVertical: 6 },
    heading3: { fontSize: ms(15), fontWeight: '600', color: Colors.textPrimary, marginVertical: 4 },
    ...
    th: { padding: 8, borderWidth: 1, borderColor: Colors.border, backgroundColor: '#f1f5f9' },
...
const StreamingCursor = () => {
    ...
    return (
        <Animated.Text style={{ opacity, color: THEME_COLOR, fontSize: ms(15), fontWeight: '700', lineHeight: 22 }}>
            ▍
        </Animated.Text>
    );
};
    ...
    const handleSend = async (text = inputText) => {
        ...
        try {
            await aiAPI.sendMessageStreamXHR(text.trim(), currentSessionId, {
                ...
                onError: (message) => {
                    setMessages(prev => {
                        ...
                        if (last && last.sender === 'ai') {
                            next[next.length - 1] = { ...last, text: message || 'An error occurred.', isError: true, streaming: false };
                        }
                        return next;
                    });
                },
            ...
            setMessages(prev => {
                ...
                const last = next[next.length - 1];
                if (last && last.sender === 'ai') {
                    next[next.length - 1] = { ...last, text: error.message || 'Network error. Please check your connection.', isError: true, streaming: false };
                }
                return next;
            });
            setIsStreaming(false);
                    ...
                    <View style={[styles.messageBubble, isUser ? styles.messageBubbleUser : styles.messageBubbleAi]}>
                        {isAiEmpty ? (
                            ...
                            <View style={styles.errorBubble}>
                                <Icon name="alert-circle-outline" size={ms(14)} color={Colors.error} style={{ marginRight: 6 }} />
                                <AppText size={13} color={Colors.error} style={{ flex: 1, lineHeight: 20 }}>{item.text}</AppText>
                            </View>
                        ) : isUser ? (
                            <AppText size={14} color={Colors.white} style={{ lineHeight: 22 }}>{item.text}</AppText>
                        ) : (
                            <View>
                                <Markdown
    ...
    const renderWelcome = () => (
        <Pressable style={styles.contentContainer} onPress={Keyboard.dismiss}>
            <View style={styles.welcomeContainer}>
                ...
                <AppText size="sm" color={Colors.textMuted} style={{ textAlign: 'center', marginTop: 8 }}>
            ...
            <View style={styles.suggestionsContainer}>
                {suggestions.map((item) => (
                    <TouchableOpacity
                        ...
                        activeOpacity={0.75}
                    >
                        <View style={[styles.suggestionIcon, { backgroundColor: item.color + '18' }]}>
                            <Icon name={item.icon} size={ms(18)} color={item.color} />
                        </View>
                        <AppText style={styles.suggestionText}>{item.text}</AppText>
                        <Icon name="chevron-forward" size={ms(14)} color={Colors.textMuted} />
                    </TouchableOpacity>
                ))}
            </View>
        </Pressable>
            ...
            <View style={styles.creditBar}>
                <View style={styles.creditRow}>
                    <View style={styles.creditLabelRow}>
                        <Icon name="flash" size={ms(13)} color={isLow ? Colors.error : THEME_COLOR} />
                        <AppText weight="semiBold" size="xs" style={{ marginLeft: 4 }}>{planName || 'Basic'}</AppText>
                        <AppText size="xs" color={Colors.textMuted} style={{ marginLeft: 8 }}>
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <View style={styles.header}>
                <View style={styles.headerLeft}>
                    ...
                    <View>
                        <AppText size="base" weight="bold">AI Assistant</AppText>
                        <View style={styles.statusContainer}>
                            {isStreaming ? (
                                <>
                                    <ActivityIndicator size="small" color={THEME_COLOR} style={{ marginRight: 4, transform: [{ scale: 0.65 }] }} />
                                    <AppText size="xs" color={THEME_COLOR}>Generating…</AppText>
                                </>
                            ) : isLoading ? (
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                            <Icon name="time-outline" size={ms(20)} color={Colors.textPrimary} />
                            <AppText size="lg" weight="bold">Chat History</AppText>
                        </View>
                        <TouchableOpacity onPress={() => setIsHistoryVisible(false)} style={styles.headerButton}>
                    ...
                    <TouchableOpacity style={styles.newChatBtn} onPress={handleNewChat} activeOpacity={0.8}>
                        <Icon name="add-circle-outline" size={ms(18)} color={Colors.white} />
                        <AppText size="sm" weight="semiBold" color={Colors.white} style={{ marginLeft: 8 }}>
                            New Chat
                        </AppText>
                    </TouchableOpacity>
                    ...
                    {sessions.length === 0 ? (
                        <View style={styles.emptyHistory}>
                            <Icon name="chatbubble-outline" size={ms(44)} color={Colors.textMuted} />
                            <AppText color={Colors.textMuted} style={{ marginTop: 12 }}>No chat history yet</AppText>
                        </View>
                    ) : (
                        <FlatList
                            data={sessions}
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity
                                    style={[styles.historyItem, currentSessionId === item._id && styles.historyItemActive]}
                                    onPress={() => fetchHistory(item._id)}
                                    activeOpacity={0.75}
                                >
                                    <View style={styles.historyIconWrap}>
                                        <Icon name="chatbubble-ellipses-outline" size={ms(16)} color={currentSessionId === item._id ? THEME_COLOR : Colors.textMuted} />
                                    </View>
                                    ...
                                    >
                                        <Icon name="trash-outline" size={ms(18)} color={Colors.textMuted} />
                                    </TouchableOpacity>
                                </TouchableOpacity>
                            )}
                        />
