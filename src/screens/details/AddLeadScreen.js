/**
 * AddLeadScreen
 * Full lead creation form – mirrors every field & API integration from LeadModal.tsx (web)
 * UI: Premium design with KeyboardAvoidingView for proper keyboard handling
 */
...
import { AppText, AppInput, AppButton, ModalLoader, SearchablePicker } from '../../components';
import {
    leadsAPI,
    productsAPI,
    dealStagesAPI,
...
const AddLeadScreen = ({ navigation }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        companyId: 'none',
        productId: 'none',
        userId: canEditSalesperson ? 'none' : (user?._id || user?.id || 'none'),
        telesalesId: 'none',
        pipelineId: 'none',
        stage: 'none',
        value: '',
    ...
    // ── Computed values ──
    const singlePipelineId = useMemo(() => {
        if (pipelines.length !== 1) return null;
        return pipelines[0]._id || pipelines[0].id || null;
    }, [pipelines]);

    const effectivePipelineId = singlePipelineId || (formData.pipelineId !== 'none' ? formData.pipelineId : null);
    ...
    }, [dealStages, effectivePipelineId, pipelines.length]);
    ...
    // ── Auto-set pipeline default ──
    useEffect(() => {
        if (pipelines.length === 1) {
            const pid = pipelines[0]._id || pipelines[0].id || 'none';
            setFormData(prev => {
                if (prev.pipelineId !== 'none') return prev;
                const stages = dealStages.filter(s => {
                    const sp = s.pipeline || s.pipelineId;
                    return sp && String(sp) === String(pid);
                });
                const defaultStage = stages.length > 0
                    ? (stages[0]._id || stages[0].id || 'none')
                    : 'none';
                return { ...prev, pipelineId: pid, stage: defaultStage };
            });
        }
    }, [pipelines, dealStages]);
    ...
    useEffect(() => {
        const fetchAll = async () => {
            ...
            try {
                ...
                console.warn('AddLeadScreen data fetch error:', err);
            } finally {
    ...
    const addTag = tagName => {
        if (tagName && !formData.tags.includes(tagName))
            updateField('tags', [...formData.tags, tagName]);
    };
    ...
    const validateAndSave = async () => {
        ...
        if (pipelines.length >= 2 && (!effectivePipelineId || effectivePipelineId === 'none')) {
    ...
    const renderHeader = () => (
        <View style={styles.header}>
            <View style={styles.headerLeft}>
                ...
                <AppText size="xl" weight="bold" color={Colors.textPrimary}>New Lead</AppText>
            </View>
            <TouchableOpacity
            ...
            >
                {loading ? (
                    ...
                    <AppText color="#fff" weight="bold">Save</AppText>
                )}
            </TouchableOpacity>
        </View>
    );
